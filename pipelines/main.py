...
# Constants
COLLECT_DATA_SCRIPT = Path(__file__).parent / "01_collect_data.py"

# Import modules directly
spec1 = importlib.util.spec_from_file_location("collect_data", COLLECT_DATA_SCRIPT)
collect_data = importlib.util.module_from_spec(spec1)
spec1.loader.exec_module(collect_data)

spec2 = importlib.util.spec_from_file_location(
    "preprocess_data", Path(__file__).parent / "02_preprocess_data.py"
)
preprocess_data = importlib.util.module_from_spec(spec2)
spec2.loader.exec_module(preprocess_data)

spec3 = importlib.util.spec_from_file_location(
    "prepare_labeling", Path(__file__).parent / "03_prepare_labeling.py"
)
...
class PipelineOrchestrator:
    def __init__(self, data_dir: str = "data"):
        ...
        self.scraper = TiktokScraper(data_dir)
    ...
    def run_data_collection(self, force: bool = False, platform: str = "all") -> bool:
        ...
        try:
            ...
            # Run scraping with platform argument
            if platform == "tiktok":
                # Import and run TikTok scraper directly
                import subprocess
                import sys

                result = subprocess.run(
                    [sys.executable, str(COLLECT_DATA_SCRIPT), "--platform", "tiktok"],
                    capture_output=True,
                    text=True,
                    cwd=Path(__file__).parent.parent,
                )
                if result.returncode != 0:
                    logger.error(f"TikTok scraping failed: {result.stderr}")
                    return False
                logger.info(result.stdout)
                # For now, assume success and update status
                total_items = (
                    0  # We'll need to parse this from output or use a status file
                )
            elif platform == "youtube":
                # Import and run YouTube scraper directly
                import subprocess
                import sys

                result = subprocess.run(
                    [sys.executable, str(COLLECT_DATA_SCRIPT), "--platform", "youtube"],
                    capture_output=True,
                    text=True,
                    cwd=Path(__file__).parent.parent,
                )
                if result.returncode != 0:
                    logger.error(f"YouTube scraping failed: {result.stderr}")
                    return False
                logger.info(result.stdout)
                total_items = 0
            else:
                ...
                total_items = len(results.get("raw_data", []))
    ...
    def run_labeling_preparation(self, force: bool = False) -> bool:
        ...
        try:
            ...
            df_sampled = self.labeling_preparator.create_labeling_dataset(
                df, sample_size=500
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Stance Classification Pipeline Orchestrator"
    )
    parser.add_argument(
        "--stage",
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force rerun completed stages"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="all",
        help="Platform to scrape for collection stage (default: all)",
    )
    ...
    if args.stage == "collect":
        success = orchestrator.run_data_collection(
            force=args.force, platform=args.platform
        )
